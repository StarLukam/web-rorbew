...
async function getProductsByGame(slug: string): Promise<Product[]> {
  ...
  return ((data || []) as Product[]).filter(
    (product) => gameSlug(product.game || "") === slug
  );
}
...
export default async function GamePage({
  ...
  const rawProducts = await getProductsByGame(slug);
  ...
  const title =
    rawProducts[0]?.game ||
    (slug === "the-spike" ? "The Spike" : gameNameFromSlug(slug));

  const promoUrl =
    promoOnly === true
      ? `/games/${slug}?status=${status}&sort=${sort}`
      : `/games/${slug}?status=${status}&sort=${sort}&promo=true`;
